...
def simulate_strokes(geometry: Dict[str, Any], iterations: int = 5000) -> Dict[str, Any]:
    # Local RNG instance to avoid global state side-effects
    ...
    for _ in range(iterations):
        ...
        tee_disp = 0.08 * 270
        tee_miss = rng.gauss(0, tee_disp)

        app_dist = dist - 270
        app_disp = 0.06 * app_dist
        app_miss = rng.gauss(0, app_disp)

        penalty = 0
        if abs(tee_miss) > 12 or abs(app_miss) > 10 or rng.random() < haz_prob:
            penalty = 1

        putts = 2 if rng.random() > 0.1 else 3

        strokes = par + penalty + (rng.randint(-1, 2))
