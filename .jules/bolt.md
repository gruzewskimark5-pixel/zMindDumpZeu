...
## 2024-05-24 - Avoid expensive logger.exception() for expected errors
**Learning:** In Python high-throughput hot paths, using `logger.exception()` for expected validation errors causes significant performance degradation (~2x slower) due to expensive stack trace generation.
**Action:** Catch specific expected exceptions like `InvalidZPulseInputError` and use standard logging (e.g., `logger.warning()` or `logger.error()`) without tracebacks instead. Reserve `logger.exception()` only for truly unexpected errors.
