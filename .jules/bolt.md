...
## 2024-06-25 - Python Exception Logging & Traceback Optimization
**Learning:** Using `logger.exception()` for expected validation errors causes significant performance degradation due to expensive stack trace generation. In hot paths, this can slow down error handling code significantly.
**Action:** Catch specific expected exceptions (like custom validation errors) and use standard logging (e.g., `logger.warning()`) without tracebacks, reserving `logger.exception()` for truly unexpected errors.
